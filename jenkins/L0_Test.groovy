...
def renderTestDB(turtleBinPath, testContext, llmSrc, stageName) {
    def makoOpts = ""
    def scriptPath = "${llmSrc}/tests/llm-test-defs/turtle/defs/sysinfo/get_sysinfo.py"
    ...
    def testDBPath = "${llmSrc}/tests/llm-test-defs/turtle/test_lists/test-db"
...
def runLLMTestlistOnPlatformImpl(pipeline, platform, testList, config=VANILLA_CONFIG, perfMode=false, stageName="Undefined", testMethod="Turtle", splitId=1, splits=1, skipInstallWheel=false, cpver="cp312")
{
    // Step 1: create LLM_ROOT dir and clone TURTLE
    sh "pwd && ls -alh"
    def llmRootConfig = "${LLM_ROOT}${config}"
    def turtleRoot = "${TURTLE_ROOT}${config}"
    sh "mkdir ${llmRootConfig}"
    trtllm_utils.checkoutSource(TURTLE_REPO, TURTLE_BRANCH, turtleRoot, true, true)
    sh "echo pandas >> ${turtleRoot}/requirements.txt"

    def llmPath = sh (script: "realpath ${llmRootConfig}",returnStdout: true).trim()
    def turtleBinPath = sh (script: "realpath ${turtleRoot}/bin/turtle",returnStdout: true).trim()
    def llmSrc = "${llmPath}/TensorRT-LLM/src"
    ...
    stage ("[${stageName}] Run ${testMethod}")
    {
        echoNodeAndGpuInfo(pipeline, stageName)
        ...
        def testCmdLine = []
        if (testMethod == "Turtle")
        {
            // Turn on verbose and -s to ease the debug and performance analysis for the job, can turn off later
            testCmdLine = [
                "LLM_ROOT=${llmSrc}",
                ...
                "-D ${llmSrc}/tests/llm-test-defs/turtle/defs",
                "-f ${llmSrc}/tests/llm-test-defs/turtle/test_lists/bloom/${testList}.txt",
                "--waives-file ${llmSrc}/tests/llm-test-defs/turtle/test_lists/waives.txt",
                "--test-timeout",
                "3600",
                "--test-python3-exe /usr/bin/python3",
                "--junit-xml",
                "--gpu-disable-clock-locking",
                "--perf",
                "--perf-clock-gpu-configs-file ${llmSrc}/tests/llm-test-defs/turtle/perf_configs/gpu_configs.yml",
            ...
            def testDBList = renderTestDB(turtleBinPath, testList, llmSrc, stageName)
            ...
            testCmdLine = [
                ...
                "--rootdir ${llmSrc}/tests/llm-test-defs/turtle/defs",
                "--test-prefix=${stageName}",
                "--splits ${splits}",
                "--group ${splitId}",
                "--waives-file=${llmSrc}/tests/llm-test-defs/turtle/test_lists/waives.txt",
        ...
        withEnv(["LD_LIBRARY_PATH=${containerLD_LIBRARY_PATH}"]) {
            withCredentials([
                ...
                trtllm_utils.llmExecStepWithRetry(
                    ...
                    script: """
                        rm -rf ${stageName}/ && \
                        cd ${llmSrc}/tests/llm-test-defs/turtle/defs && \
                        ${testCmdLine.join(" ")}
                    """,
        ...
        if (perfMode) {
            stage("Check perf result") {
                sh """
                    python3 ${llmSrc}/tests/llm-test-defs/turtle/defs/perf/sanity_perf_check.py \
                    ${stageName}/perf_script_test_results.csv \
                    ${llmSrc}/tests/llm-test-defs/turtle/defs/perf/base_perf.csv
                """
            }
        }
    }
...
def runLLMTestlistOnPlatform(pipeline, platform, testList, config=VANILLA_CONFIG, perfMode=false, stageName="Undefined", testMethod="Turtle", splitId=1, splits=1, skipInstallWheel=false, cpver="cp312")
{
    cacheErrorAndUploadResult(stageName, {
        runLLMTestlistOnPlatformImpl(pipeline, platform, testList, config, perfMode, stageName, testMethod, splitId, splits, skipInstallWheel, cpver)
    }, {
        def llmPath = sh (script: "realpath .", returnStdout: true).trim()
...
def checkPipInstall(pipeline, wheel_path)
{
    ...
    trtllm_utils.llmExecStepWithRetry(pipeline, script: "cd ${LLM_ROOT}/tests/ && python3 test_pip_install.py --wheel_path ${wheelArtifactLinks}")
}
...
def runPackageSanityCheck(pipeline, wheel_path, reinstall_dependencies=false, cpver="cp312")
{
    ...
    // Test preview installation
    trtllm_utils.llmExecStepWithRetry(pipeline, script: "#!/bin/bash \n" + "pip3 install pytest tensorrt_llm-*.whl")
    if (env.alternativeTRT) {
        sh "bash -c 'pip3 show tensorrt || true'"
    }
    ...
    sh "#!/bin/bash \n" + "cd tensorrt_llm/examples/gpt && python3 ../generate_checkpoint_config.py --architecture GPTForCausalLM --dtype float16" +
    " && trtllm-build --model_config config.json --log_level verbose &&  python3 ../run.py  --max_output_len 4 --end_id -1"
}
...
def launchTestJobs(pipeline, testFilter, dockerNode=null)
{
    ...
    turtleConfigs = [
        ...
        "H100_PCIe-[Post-Merge]-1": ["h100-cr", "l0_h100", 1, 2],
        "H100_PCIe-[Post-Merge]-2": ["h100-cr", "l0_h100", 2, 2],
        "DGX_H100-4_GPUs-[Post-Merge]": ["dgx-h100-x4", "l0_dgx_h100", 1, 1, 4],
    ]

    parallelJobs = turtleConfigs.collectEntries{key, values -> [key, [createKubernetesPodConfig(LLM_DOCKER_IMAGE, values[0], "amd64", values[4] ?: 1), {
        ...
        runLLMTestlistOnPlatform(pipeline, values[0], values[1], config, false, key, "Pytest", values[2], values[3])
    }]]}

    e2ePerfConfigs = [
        "A100_80GB_PCIE-e2e-perf": "a100-80gb-pcie",
        "H100_PCIe-e2e-perf": "h100-cr",
    ]
    parallelJobs += e2ePerfConfigs.collectEntries{key, value -> [key, [createKubernetesPodConfig(LLM_DOCKER_IMAGE, value, "amd64", 1, true), {
        runLLMTestlistOnPlatform(pipeline, value, "l0_e2e_perf", VANILLA_CONFIG, true, key)
    }]]}
    ...
    if (env.targetArch == AARCH64_TRIPLE) {
        parallelJobs = aarch64Configs.collectEntries{key, values -> [key, [createKubernetesPodConfig(LLM_DOCKER_IMAGE, values[0], "arm64"), {
            runLLMTestlistOnPlatform(pipeline, values[0], values[1], LINUX_AARCH64_CONFIG, false, key, "Pytest", values[2], values[3])
        }]]}
    }
            ...
            if (checkPipStage) {
                stage("Run LLMAPI tests") {
                    ...
                    trtllm_utils.launchKubernetesPod(pipeline, pipInstallSanitySpec, "trt-llm", {
                        ...
                        withEnv(libEnv) {
                            runLLMTestlistOnPlatform(pipeline, gpu_type, "l0_sanity_check", config, false, "${values[1]}-${key}-sanity-check" , "Pytest", 1, 1, true, null)
                        }
                    })
                }
            }
    ...
    multiGpuJobs = parallelJobs.findAll{it.key.contains("4_GPUs")}
    ...
    if (testFilter[TEST_STAGE_LIST] != null) {
        ...
        parallelJobs = parallelJobs.findAll {it.key in testFilter[(TEST_STAGE_LIST)] || it.key in testFilter[(EXTRA_STAGE_LIST)]}
        println parallelJobs.keySet()
    }
    if (testFilter[(GPU_TYPE_LIST)] != null) {
        echo "Use GPU_TYPE_LIST for filtering."
        parallelJobs = parallelJobs.findAll {it.key.tokenize('-')[0] in testFilter[(GPU_TYPE_LIST)]}
        println parallelJobs.keySet()
    }
    if (testFilter[(ENABLE_SKIP_TEST)]) {
        echo "All test stages are skipped."
        parallelJobs = docBuildJobs + sanityCheckJobs
    }
    if (testFilter[(MULTI_GPU_FILE_CHANGED)] == false) {
        parallelJobs = parallelJobs.findAll {!(it.key in multiGpuJobs.keySet())}
        forceRunJobs = multiGpuJobs.findAll {it.key in testFilter[(TEST_STAGE_LIST)] || it.key in testFilter[(EXTRA_STAGE_LIST)]}
        parallelJobs += forceRunJobs
    }
    if (env.alternativeTRT || testFilter[(ENABLE_MULTI_GPU_TEST)] || testFilter[(IS_POST_MERGE)]) {
        echo("Force run multi-GPU testing.")
        parallelJobs += multiGpuJobs
    }

    postMergeJobs = parallelJobs.findAll {it.key.contains("Post-Merge")}
    // As default, we don't run post-merge tests in pre-merge CI
    parallelJobs -= postMergeJobs
    if (env.alternativeTRT || testFilter[(IS_POST_MERGE)]) {
        parallelJobs += postMergeJobs
    } else {
        postMergeJobsFiltered = [:]
        postMergeJobsFiltered = postMergeJobs.findAll {it.key in testFilter[(EXTRA_STAGE_LIST)] || it.key in testFilter[(TEST_STAGE_LIST)]}
        parallelJobs += postMergeJobsFiltered
    }
    ...
    def keysStr = parallelJobs.keySet().join(",\n")
    pipeline.echo "Now we will run stages: [\n${keysStr}\n]"

    parallelJobs = parallelJobs.collectEntries { key, values -> [key, {
        stage(key) {
